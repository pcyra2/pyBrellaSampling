...
def main():
    ...
    assert type(int(verbosity)) is int, f"ERROR: verbosity statement must be an integer, not {verbosity}"
    ...
    if args["Stage"].casefold() == "pull":
        pull(Umbrella, MM, QM, Calc, StartFile=args["StartFile"])
...
def pull(Umbrella: UmbrellaClass, MM: MMClass, QM: QMClass, Calc: CalcClass, StartFile: str):
    ...
    if Umbrella.Start < Umbrella.BinVals[0]:
        Umbrella.add_start(0)
    elif Umbrella.Start > Umbrella.BinVals[-1]:
        Umbrella.add_start(len(Umbrella.BinVals)-1 )
    else:
    ...
    for i in range(Umbrella.Bins):
        if i < Umbrella.StartBin:
            ...
            if os.path.isfile("../heat_1.0.restart.coor"):
