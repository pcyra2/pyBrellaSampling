...
def genMol(atoms: str, charge: int, spin:int, basis: str, symmetry:bool)->pyscf.M:
...
def UHF(mol: pyscf.M, charges=None, locs=None)->pyscf.scf.UHF:
    ...
    HF.max_cycle = 300
    if charges != None:
        mf = pyscf.qmmm.mm_charge(HF, locs, charges)
        mf.kernel()
...
def get_gradients(mf: pyscf.scf.uhf.UHF)->numpy.ndarray:
    """
    Calculate the gradients of the energy with respect to nuclear coordinates.

    Parameters:
        mf (pyscf.scf.uhf.UHF): The mean-field object representing the molecular system.

    Returns:
        numpy.ndarray: The gradients of the energy with respect to nuclear coordinates.
    """
